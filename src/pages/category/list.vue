...
<script>
import { category, deleteCategory } from '@/services/category'
export default {
  data () {
    return {
      obj: {
  ...
  methods: {
    onChange () {
      ...
      // checked === true ? this.obj.status = 1 : this.obj.status = 0
    },
    onClose () {
      this.visible = false;
    },
    ...
    handleUpdate (row) { //处理修改
      this.obj = Object.assign({}, row)  //复制对象
      this.obj.status == 1 ? this.checked = true : this.checked = false
      this.dialogStatus = 'update'   //修改值当点击确认调用updateDate（）
      this.visible = true;//弹出
